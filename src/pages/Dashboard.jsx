...
export default function Dashboard({ t, language, onToggleLanguage }) {
  ...
  const optionLabels = useMemo(
    () =>
      optionKeys.reduce((labels, key) => {
        const projectOption = project.options[key];
        if (projectOption.labelKey && t.options[projectOption.labelKey]) {
          labels[key] = t.options[projectOption.labelKey];
        }
        return labels;
      }, {}),
    [optionKeys, project.options, t.options],
  );
      ...
      <main ref={sectionRef} className="max-w-7xl mx-auto px-6 pb-16" style={{ backgroundColor: isFullscreen ? "#151522" : "transparent" }}>
        ...
        <div ref={exportRef}>
          {/* ---------------------------------------------------------------
              ...
              first - real market metrics only, no Madinum-internal figures.
          --------------------------------------------------------------- */}
